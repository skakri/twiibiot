...
class Twitter
{
	 ...
	 * @throws TwitterException
	 * INFO: extended by Kristaps Karlsons to add support for in_reply_to_status_id
	 */
	public function send($message, $in_reply_to = false)
	{
		...
		if (!$in_reply_to) {
			...
			$res = $this->request('statuses/update', array('status' => $message, 'in_reply_to_status_id' => $in_reply_to));
		}
